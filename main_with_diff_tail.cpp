...
void find_alternative_tail(list<int> &comp_path, vector<vector<int>> &comp_graph, vector<int> &weights, vector<int> &seen, vector<bool> &vis, vector<int> &seen_pref, vector<bool> &vis_pref, clock_t &time_start, float &time_limit, bool random=false, bool backtrack=false){
    int pref_value = 0;
    int curr_value = 0;
    for(int v : comp_path)
    ...
    {   
        see_node(*tail_start, comp_graph, seen_pref, vis_pref);
        list<int> tail = {*tail_start};
        search_for_tail(tail, comp_graph, weights, seen_pref, vis_pref, random, backtrack);
        ...
        if(tail_value >= curr_value - pref_value - weights[*tail_start])
        {
            unsee_path(next(tail_start), comp_path.end(), comp_graph, seen, vis);
            unsee_path(tail, comp_graph, seen_pref, vis_pref);
        ...
        else unsee_path(tail, comp_graph, seen_pref, vis_pref);
        
        pref_value += weights[*tail_start];

        //print_final_path(comp_path);
    }
    // unsee_path(comp_path, comp_graph, seen_pref, vis_pref);
    vis_pref.assign(comp_graph.size(), false);
    seen_pref.assign(comp_graph.size(), 0);
...
void call_main_search(int N, vector<vector<int>> &graph, vector<vector<int>> &comp_graph, vector<int> &weights, list<int> &final_path, vector<int> &which_comp, vector<CompNode> &comp_nodes, list<int> &comp_path, clock_t &time_start, int type){
    float time_limit = 19.2f;
    vector<bool> vis(comp_graph.size(), false);
    vector<int> seen(comp_graph.size(), 0);
    vector<int> pref_value(comp_graph.size(), 0);
    vector<int> seen_pref(comp_graph.size(), 0);
    vector<bool> vis_pref(comp_graph.size(), false);
    ...
    while(check_time(time_limit, time_start))
    {
        ...
        {
        case 3:
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, seen_pref, vis_pref, time_start, time_limit); //budowanie alternatywnego taila dla każdego wierzchołka z ścieżki po kolei
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, seen_pref, vis_pref, time_start, time_limit, true); //budowanie alternatywnego taila dla każdego wierzchołka z ścieżki po kolei
        ...
        case 7:
            ...
            break;
        case 5:
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, seen_pref, vis_pref, time_start, time_limit, false, true); //budowanie alternatywnego taila dla każdego wierzchołka z ścieżki po kolei
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, seen_pref, vis_pref, time_start, time_limit, true, true); //budowanie alternatywnego taila dla każdego wierzchołka z ścieżki po kolei
        ...
        case 6:
            ...
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, seen_pref, vis_pref, time_start, time_limit); //budowanie alternatywnego taila dla każdego wierzchołka z ścieżki po kolei
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, seen_pref, vis_pref, time_start, time_limit, true); //budowanie alternatywnego taila dla każdego wierzchołka z ścieżki po kolei
            find_detours(comp_path, comp_graph, weights, seen, vis, pref_value, time_start, time_limit, 10, true, true); //szukanie objazdu
            extend_all(comp_path, comp_graph, weights, seen, vis); //poszerzanie końców
            break;
        case 2:
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, seen_pref, vis_pref, time_start, time_limit); //budowanie alternatywnego taila dla każdego wierzchołka z ścieżki po kolei
