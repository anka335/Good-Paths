...
int call_start_search(int N, list<int> &comp_path, vector<vector<int>> &graph, vector<vector<int>> &comp_graph, vector<int> &weights, vector<int> &which_comp, vector<CompNode> &comp_nodes, int type, clock_t &time_start, float time_limit=6.5f)
{
    ...
    while(check_time(time_limit, time_start))
    {
        ...
        if(curr_result > best_result || comp_path.empty())
        {
...
void backtrack(int v, vector<bool> &visited, vector<vector<int>> &comp_graph, vector<int> &weights, vector<int> &seen, vector<int> &curr_path, vector<int> &best_path, clock_t &last_update, int &best_weight, int threshold=0, int last_common=0, float timeout=19.0f, int curr_weight=0){
    if(!check_time(timeout, last_update) && curr_weight < best_weight){
        cout << "RETURNING DUE TO TIMEOUT\n";
        if(best_path.empty()){
        ...
        cout << "BEST WEIGHT FOUND: " << best_weight << '\n';
        return;
    }
...
void call_backtrack(list<int> &path, vector<vector<int>> &comp_graph, vector<int> &weights, clock_t &time_start, float time_limit=19.0f, float timeout=0.5f){
    ...
    //int elapses = 0;
    int start;
    cout << "time limit for backtrack: " << time_limit << " time start : " << ((float)(clock()-time_start)) / CLOCKS_PER_SEC << '\n';
    while(check_time(time_limit, time_start))
    {
        do
        {
...
void call_main_search(int N, vector<vector<int>> &graph, vector<vector<int>> &comp_graph, vector<int> &weights, list<int> &final_path, vector<int> &which_comp, vector<CompNode> &comp_nodes, list<int> &comp_path, clock_t &time_start, int type, float time_limit=19.1f){
    ...
    while(check_time(time_limit, time_start))
    {
        ...
        {
        case 3:
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, false, false); //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, true, false); //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
            find_detours(comp_path, comp_graph, weights, seen, vis, pref_value, time_start, time_limit, 9000, true, true); //szukanie objazdu
            extend_all(comp_path, comp_graph, weights, seen, vis); //poszerzanie koï¿ï¾cï¿ï¾³w
            break;
        case 7:
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, false, false);
            //find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, false, true);  //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, true, false); //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
        ...
        case 4:
            ...
            // find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, false, true);  //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, true, false); //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
            find_detours(comp_path, comp_graph, weights, seen, vis, pref_value, time_start, time_limit, 10000, false, true); //szukanie objazdu
            extend_all(comp_path, comp_graph, weights, seen, vis); //poszerzanie koï¿ï¾cï¿ï¾³w
            break;
        case 1:
            //print_final_path(comp_path);
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, false, false);
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, false, true);  //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
            // find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, true); //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
            find_detours(comp_path, comp_graph, weights, seen, vis, pref_value, time_start, time_limit, 25000, true, true); //szukanie objazdu
            extend_all(comp_path, comp_graph, weights, seen, vis); //poszerzanie koï¿ï¾cï¿ï¾³w
            break;
        case 6:
            //print_final_path(comp_path);
            ...
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, false, true);  //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
            // find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, true); //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
            find_detours(comp_path, comp_graph, weights, seen, vis, pref_value, time_start, time_limit, 100, true, true); //szukanie objazdu
            extend_all(comp_path, comp_graph, weights, seen, vis); //poszerzanie koï¿ï¾cï¿ï¾³w
            break;
        case 2:
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, false, false);
            find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, false, true);  //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
            // find_alternative_tail(comp_path, comp_graph, weights, seen, vis, time_start, time_limit, true); //budowanie alternatywnego taila dla kaï¿ï¾¼dego wierzchoï¿ï¾ka z ï¿ï¾cieï¿ï¾¼ki po kolei
            find_detours(comp_path, comp_graph, weights, seen, vis, pref_value, time_start, time_limit, 10000, true, true); //szukanie objazdu
            extend_all(comp_path, comp_graph, weights, seen, vis); //poszerzanie koï¿ï¾cï¿ï¾³w
        }
    }
}
...
void find_solution(int N, vector<vector<int>> &graph, vector<vector<int>> &comp_graph, vector<int> &weights, list<int> &final_path, vector<int> &which_comp, vector<CompNode> &comp_nodes, int type, clock_t &time_start)
{
    ...
    //get_diameter(comp_graph, final_path, weights);
    call_start_search(N, final_path, graph, comp_graph, weights, which_comp, comp_nodes, type, time_start, 300.0f);

   /* if(type == 4){
        // cerr << comp_graph.size() << '\n';
        sort_graph(comp_graph, weights);
        // call_start_search(N, comp_path, graph, comp_graph, weights, which_comp, comp_nodes, type, time_start, 15.5f);
        call_backtrack(comp_path, comp_graph, weights, time_start, 140.0f, 1.0f);
        call_main_search(N, graph, comp_graph, weights, final_path, which_comp, comp_nodes, comp_path, time_start, type, 170.0f);
    ...
    {
        sort_graph(comp_graph, weights);
        call_backtrack(comp_path, comp_graph, weights, time_start, 149.0f, 1.0f);
        // cerr << comp_path.size() << '\n';
        call_main_search(N, graph, comp_graph, weights, final_path, which_comp, comp_nodes, comp_path, time_start, type, 170.0f);
        // cerr << comp_path.size() << '\n';
        decompress_branches(N, graph, comp_graph, weights, comp_path, final_path, which_comp, comp_nodes);
    ...
    else if(type == 1)
    {
        ...
        // sort_graph(comp_graph, weights);
        call_start_search(N, comp_path, graph, comp_graph, weights, which_comp, comp_nodes, type, time_start, 32.0f);
        call_main_search(N, graph, comp_graph, weights, final_path, which_comp, comp_nodes, comp_path, time_start, type, 170.0f);
        final_path = comp_path;
        //decompress_branches(N, graph, comp_graph, weights, comp_path, final_path, which_comp, comp_nodes);
    }
